...
import { FieldCompProcessor, type FieldCompConfig } from './fieldcomp-processor'
...
class MeterProcessor extends AudioWorkletProcessor {
  ...
  private fieldComp: FieldCompProcessor
  ...
  constructor() {
    ...
    this.fieldComp = new FieldCompProcessor(fs)
    ...
    this.port.onmessage = (event: MessageEvent) => {
      this.fieldComp.updateConfig((event.data || {}) as FieldCompConfig)
    }
  }
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
    ...
    for (let i = 0; i < n; i++) {
      const inputL = L[i]
      const inputR = R[i]
      const [l, r] = this.fieldComp.processFrame(inputL, inputR)
