...
// - fieldcomp.cpp由来のコンプレッサー / オートメイクアップ / ルックアヘッドリミッター
//
...
class MeterProcessor extends AudioWorkletProcessor {
  ...
  compressFrame(left: number, right: number): [number, number] {
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
    ...
    for (let i = 0; i < n; i++) {
      const inputL = L[i]
      const inputR = this.monoMicEnabled ? L[i] : R[i]
      const [l, r] = this.compressorEnabled
        ? this.compressFrame(inputL, inputR)
        : [inputL, inputR]
